...
if (isset($_GET['action'])) {
    ...
    $servicioenproceso = new ServiciosProcesoData;
    ...
    if (isset($_SESSION['idUsuarioCliente'])) {
        ...
        switch ($_GET['action']) {
            case 'readCarrosenProceso':
                if (
                    !$servicioenproceso->setIdServicio($_POST['id_servicio'])
                ) {
                    $result['error'] = $servicioenproceso->getDataError();
                } elseif ($result['dataset'] = $servicioenproceso->readCarrosenProceso()) {
                    $result['status'] = 1;
                } else {
